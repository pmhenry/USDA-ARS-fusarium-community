...
## Paths, Data file
vInfile_counts   <- file.path(vDir_data_prep, "data_EF", "EF_count.csv")    ## count
vInfile_distance <- file.path(vDir_data_prep, "data_EF", "EF_distance.phy") ## distance
vInfile_taxa     <- file.path(vDir_data_prep, "data_EF", "EF_taxonomy.csv") ## taxa
vInfile_sample   <- file.path(vDir_data_prep, "data_EF", "EF_sample.csv")   ## sample
...
## Phyloseq object: Non-normalized (264 samples x 34 taxa)
physeq_raw <- phyloseq(otu_table(dfCounts, taxa_are_rows=F), ## Counts
...
par(mar = c(12, 4, 4, 2) + 0.1) ## make more room on bottom margin
barplot(sort(taxa_sums(physeq_norm), TRUE)[1:N]/nsamples(physeq_norm)*100, las=2, cex.names=.75, main="Rank-abundance Oligotype Barplot, 1% cutoff", ylab="relative abundance (%)", mgp = c(2, .5, 0))
lines(c(-50,100),c(cutoff, cutoff), lty = "dotted", ylab="0.01", col="red")

## Create phyloseq object with only top N oligotypes included (14 identified above 1% across samples)
TopNOTUs <- names(sort(taxa_sums(physeq_norm), TRUE)[1:14]) 
physeq_toptax <- prune_taxa(TopNOTUs, physeq_norm)

## Bar Graph, PLANT Individual Replicates (FIGURE 1.1, legend)
...
vPhy_Bar <- plot_bar(physeq_toptax_plnt, x="repetition", y="Abundance", fill="taxa_OTU")+
...
vPhy_Bar
fExportSVG(vPhy_Bar, vDirPhyloseq, "fig_1.1_EF_barplot_plnt_legend.svg", vfW=vW_bar, vfH=vH_bar)

## Bar Graph, PLANT Individual Replicates (FIGURE 1.1, no legend)
mycolors <- c("#850027", "#c12826", "#aa4f2b", "#ca7c4a", "#feb70e", "#ece372", "#1bd17c", "#628c46", "#664e93", "#ca8096", "#6a1154", "#375670", "#1185c0", "#3e30ff")
physeq_toptax_plnt <- prune_samples(sample_data(physeq_toptax)$extracted_from=="plnt", physeq_toptax)
vPhy_Bar <- plot_bar(physeq_toptax_plnt, x="repetition", y="Abundance", fill="taxa_OTU")+
  facet_grid(experiment~treatment, scales="free_x")+
  ...
  scale_y_continuous(labels=function(x)x*100)
vPhy_Bar
fExportSVG(vPhy_Bar, vDirPhyloseq, "fig_1.1_EF_barplot_plnt_nolegend.svg", vfW=vW_bar, vfH=vH_bar)

## Bar Graph, SOIL Individual Replicates (FIGURE 2.1, legend)
mycolors <- c("#850027", "#c12826", "#aa4f2b", "#ca7c4a", "#feb70e", "#ece372", "#1bd17c", "#628c46", "#664e93", "#ca8096", "#6a1154", "#375670", "#1185c0", "#3e30ff")
...
vPhy_Bar <- plot_bar(physeq_toptax_plnt, x="repetition", y="Abundance", fill="taxa_OTU")+
  ...
  scale_y_continuous(labels=function(x)x*100)
vPhy_Bar
fExportSVG(vPhy_Bar, vDirPhyloseq, "fig_2.1_EF_barplot_soil_legend.svg", vfW=vW_bar, vfH=vH_bar)

## Bar Graph, SOIL Individual Replicates (FIGURE 2.1, no legend)
mycolors <- c("#850027", "#c12826", "#aa4f2b", "#ca7c4a", "#feb70e", "#ece372", "#1bd17c", "#628c46", "#664e93", "#ca8096", "#6a1154", "#375670", "#1185c0", "#3e30ff")
physeq_toptax_plnt <- prune_samples(sample_data(physeq_toptax)$extracted_from=="soil", physeq_toptax)
vPhy_Bar <- plot_bar(physeq_toptax_plnt, x="repetition", y="Abundance", fill="taxa_OTU")+
  facet_grid(experiment~treatment, scales="free_x")+
  ...
  scale_y_continuous(labels=function(x)x*100)
vPhy_Bar
fExportSVG(vPhy_Bar, vDirPhyloseq, "fig_2.1_EF_barplot_soil_nolegend.svg", vfW=vW_bar, vfH=vH_bar)
...
## PLOT: Single Plot (FIGURE 1.2, legend)
...
vPhy_ord_extract
fExportSVG(vPhy_ord_extract, vDirPhyloseq, "fig_1.2_EF_ordination_plnt_legend.svg", vfW=vW_ord, vfH=vH_ord)

## PLOT: Single Plot (FIGURE 1.2, no legend)
GP.ord_plnt <- subset_samples(physeq_norm, extracted_from=="plnt")
mycolors_7 <- c("#c12826", "#ca7c4a", "#628c46", "#48534c", "#1185c0", "#664e93", "#ca8096")
vPhy_ord_extract <- plot_ordination(GP.ord_plnt, GP.ord, color="treatment")+
  geom_point(size=1, na.rm=T)+
  ...
  scale_fill_continuous(guide = guide_legend())
vPhy_ord_extract
fExportSVG(vPhy_ord_extract, vDirPhyloseq, "fig_1.2_EF_ordination_plnt_nolegend.svg", vfW=vW_ord, vfH=vH_ord)

## PLOT: Single Plot (FIGURE 2.2, legend)
...
vPhy_ord_extract <- plot_ordination(GP.ord_soil, GP.ord, color="treatment")+
  ...
  scale_fill_continuous(guide = guide_legend())
vPhy_ord_extract
fExportSVG(vPhy_ord_extract, vDirPhyloseq, "fig_2.2_EF_ordination_plnt_legend.svg", vfW=vW_ord, vfH=vH_ord)

## PLOT: Single Plot (FIGURE 2.2, no legend)
mycolors_7 <- c("#c12826", "#ca7c4a", "#628c46", "#48534c", "#1185c0", "#664e93", "#ca8096")
vPhy_ord_extract <- plot_ordination(GP.ord_soil, GP.ord, color="treatment")+
  geom_point(size=1, na.rm=T)+
  scale_colour_manual(values=mycolors_7)+
  ...
  scale_fill_continuous(guide = guide_legend())
vPhy_ord_extract
fExportSVG(vPhy_ord_extract, vDirPhyloseq, "fig_2.2_EF_ordination_plnt_nolegend.svg", vfW=vW_ord, vfH=vH_ord)
...
fExtract_comparisons <- function(vComparisons, vLevel, vSub_padj=.05, vSub_log2=1.5, vOutfile, vPhyseq, vDESeq2, vFolder, vLabel, vLabel_Fus="fusarium.xlsx") {
  "
  ...
  VARIABLES:
    ...
    > vOutfile      (string.xlsx) Output excel name
  ...
  fTryRemoveFile(file.path(vFolder, vOutfile))
  fTryRemoveFile(file.path(vFolder, vLabel_Fus))
  ...
  for (n in 1:nrow(vComparisons)) {
    ...
    vSubset <- subset(x=vSubset, vSubset$log2FoldChange >= vSub_log2 | vSubset$log2FoldChange <= -vSub_log2 ) ## Subset substantial changes
    ## Collect Fusarium data
    rownames(vDF_Fus[n,]) <- vName
    if (any(rownames(vSubset) %in% "FoxB_C2-FOF")) {
      vDF_Fus[n,] = as.data.frame(subset(vSubset, rownames(vSubset) %in% "FoxB_C2-FOF"))
    } else {
      vDF_Fus[n,] = NA
    }
    ## Export to excel
    if (nrow(vSubset)>0) {
      ...
      ## Export
      fExportXLSX(vfXLSX=vSubset, vfFolder=vFolder, vfName=vOutfile, vfSheet=vName)
      print(paste("Exported:      ",vName))
      ## PLOT
      x = tapply(vSubset$log2FoldChange, vSubset$taxa_OTU, function(x) max(x))
      x = sort(x, TRUE)
      vSubset$taxa_OTU = factor(as.character(vSubset$taxa_OTU), levels=names(x))
      vSVG <- ggplot(vSubset, aes(x=taxa_OTU, y=log2FoldChange)) + geom_point(size=4) + 
        theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5))+
        ggtitle(vName)+
        geom_hline(yintercept=0, color="red")
      fExportSVG(vSVG, vFolder, vFilename_svg)
    } else {
      ## Export
      fExportXLSX(vfXLSX="NONE", vfFolder=vFolder, vfName=vOutfile, vfSheet=vName, vfRow=F,vfCol=F)
...
## Extract Comparisons: All treatment comparisons extracted from plant
fExtract_comparisons(vComparisons = vCombos_plnt,
                     vLevel       = "group",
                     vOutfile     = "btwn_plnt.xlsx",
                     vPhyseq      = physeq_raw,
                     vDESeq2      = dds,
                     vFolder      = file.path(vDirDESeq2,"btwn_plnt"),
                     vLabel       = "btwn_plnt",
                     vLabel_Fus   = "EF_FOF.xlsx")

## Extract Comparisons: All treatment comparisons extracted from soil
fExtract_comparisons(vComparisons = vCombos_soil,
                     vLevel       = "group",
                     vOutfile     = "btwn_soil.xlsx",
                     vPhyseq      = physeq_raw,
                     vDESeq2      = dds,
                     vFolder      = file.path(vDirDESeq2,"btwn_soil"),
                     vLabel       = "btwn_soil",
                     vLabel_Fus   = "EF_FOF.xlsx")

## Extract Comparisons: All treatment comparisons extracted from brsw
fExtract_comparisons(vComparisons = vCombos_brsw,
                     vLevel       = "group",
                     vOutfile     = "btwn_brsw.xlsx",
                     vPhyseq      = physeq_raw,
                     vDESeq2      = dds,
                     vFolder      = file.path(vDirDESeq2,"btwn_brsw"),
                     vLabel       = "btwn_brsw",
                     vLabel_Fus   = "EF_FOF.xlsx")

## Extract Comparisons: All comparisons within each extraction location
fExtract_comparisons(vComparisons = vCombos_wthn,
                     vLevel       = "group",
                     vOutfile     = "wthn_extracted.xlsx",
                     vPhyseq      = physeq_raw,
                     vDESeq2      = dds,
                     vFolder      = file.path(vDirDESeq2, "wthn_extracted"),
...
pseq.rel <- microbiome::transform(physeq_raw, "compositional")
...
fPermanova <- function(vfComparisons, vfPhyloseq, vfFolder, vfName) {
  ...
  for (n in 1:nrow(vfComparisons)) {
    ...
    meta <- meta(vSubset)
    # ## Plot: PCA of Groups
    # vPermPCA <- plot_landscape(vSubset, method="NMDS", distance="bray", col="group", size=3, main=vTitle)+
    #   ggtitle(vTitle)
    # fExportSVG(vfSVG=vPermPCA, vfFolder=vfOut, vfName=vFilename_svg_PCA)
    ## Analysis: Permanova
    permanova <- adonis(phyloseq::distance(vSubset, method="wunifrac") ~ group, data = meta, permutations=999)
    ## permanova Results
